...
export default async function handler(req, res) {
    ...
    if (req.method === 'GET' && req.query.count === '1') {
        const {
            count
        } = await supabase.from('mrtinfo_platforms').select('*', {
            count: 'exact',
            head: true
        });
        return res.json({
    ...
    if (req.method === 'GET' && id) {
        const {
            data,
            error
        } = await supabase.from('mrtinfo_platforms').select('*').eq('uid', id).single();
    ...
    if (req.method === 'POST') {
        const {
            data,
            error
        } = await supabase.from('mrtinfo_platforms').insert(req.body).select().single();
    ...
    if ((req.method === 'PATCH' || req.method === 'PUT') && id) {
        const {
            data,
            error
        } = await supabase.from('mrtinfo_platforms').update(req.body).eq('uid', id).select().single();
        if (error) return res.status(400).json({
            error: error.message
        });
        return res.json(data);
    ...
    if (req.method === 'DELETE' && id) {
        const {
            error
        } = await supabase.from('mrtinfo_platforms').delete().eq('uid', id);
        if (error) return res.status(400).json({
            error: error.message
        });
